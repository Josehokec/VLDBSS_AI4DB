...
class Operator:
    def __init__(self, id, est_rows, est_cost, act_rows, task, acc_obj, exec_info, op_info, mem, disk):
        # 注意id是操作算子后面加了个数字，比如Projection_4
        self.id = id
...
class Plan:
    ...
    @staticmethod
    def parse_mysql_plan(mysql_plan_json):
        id = mysql_plan_json["id"]
        est_rows = float(mysql_plan_json["est_rows"])
        task = mysql_plan_json["task"]
        est_cost = act_rows = None
        acc_obj = mysql_plan_json["acc_obj"]
        exec_info = None
        op_info = mysql_plan_json["op_info"]
        mem = disk = None
        op = Operator(id, est_rows, est_cost, act_rows, task, acc_obj, exec_info, op_info, mem, disk)
        # 如果这个操作算子没有孩子了则直接返回
        if mysql_plan_json["children"] == None:
            return op
        # 如果这个操作算子还有孩子，则递归构造其孩子节点
        for i in range(len(mysql_plan_json["children"])):
            child = Plan.parse_mysql_plan(mysql_plan_json["children"][i])
