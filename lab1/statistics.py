...
class ExpBackoffEstimator:
    ...
    def estimate(range_query, table_stats):
        # YOUR CODE HERE
        sel = 1.0
        # create a array save sel per col 
        col_sels = [0] * len(range_query.column_names())
        for col in range_query.column_names():
            # get real data max value and min value
            min_val = table_stats.columns[col].min_val()
            max_val = table_stats.columns[col].max_val()
            # get sql max value and min value
            (left, right) = range_query.column_range(col, min_val, max_val)
            col_cnt = table_stats.columns[col].between_row_count(left+1, right)  # (left, right) -> [left, right)
            # cal this col sel
            col_sel = col_cnt / table_stats.row_count
            col_sels.append(col_sel)  
        # sort then choose top4 col_sel
        col_sels.sort(reverse=True)  
        """
        # s(1) * s(2)^(1/2) * s(3)^(1/4) * s(4)^(1/8)
        # 存在一些问题
        col_sels                                                                                            sel
        [0.3575864054752736, 0]                                                                             0.0
        [0.06860229275500808, 0]                                                                            0.0
        [0.6149429999579898, 0.40806277073399166, 0.3962244374903097, 0.046387471166533245, 0, 0, 0, 0]     0.21231650219257447
        """
        for i in range(len(col_sels)):
            # sel = col_sels[0] * (col_sels[1]**0.5) * (col_sels[2]**0.25) * (col_sels[3]**0.125)
            sel = sel * col_sels[i] ** (0.5**i)
            if i == 3:
                break
        return sel
